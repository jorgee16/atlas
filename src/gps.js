...
export class GpsController {
  constructor({ onUpdate, onStatus, autoStart = true }) {
    ...
    // remain side-effect free. The explicit GPS button still calls start/stop.
    if (
      autoStart &&
  ...
  async stop() {
    if (this.startPromise) {
      try {
        ...
        // start() already reports provider errors through onStatus.
      }
    }
  ...
  handlePosition(position) {
    ...
    if (accuracy > 150) {
      this.onStatus(
        '📍 Low GPS accuracy',
        `Current accuracy is about ${Math.round(accuracy)} m.`
      );
    }
    ...
    if (!this.courseAnchor) {
      ...
      const age = now - this.courseAnchor.timestamp;
      ...
      if (age > COURSE_MAX_FIX_AGE_MS) {
        ...
        const travelled = distanceMeters(this.courseAnchor, fix);
        const elapsedSeconds = age / 1000;
        const observedSpeed = travelled / elapsedSeconds;
        const anchorAccuracy = Number.isFinite(this.courseAnchor.accuracy)
          ? this.courseAnchor.accuracy
          : accuracy;
        const uncertainty = Math.max(accuracy, anchorAccuracy);
        const requiredDistance = Math.max(
          MOTION_MIN_DISTANCE_METERS,
          uncertainty * MOTION_ACCURACY_DISTANCE_FACTOR
        );
        const providerSpeed =
          Number.isFinite(speed) && speed >= 0 ? speed : null;
        const plausibleObservedSpeed =
          providerSpeed === null ||
          observedSpeed <= Math.max(
            MOTION_SPEED_PLAUSIBILITY_FLOOR,
            providerSpeed * MOTION_SPEED_PLAUSIBILITY_MULTIPLIER
          );
        const movementConfirmed =
          travelled >= requiredDistance &&
          observedSpeed >= MOTION_MIN_SPEED_METERS_PER_SECOND &&
          plausibleObservedSpeed;

        if (movementConfirmed) {
          const movementHeading = bearingBetween(this.courseAnchor, fix);
          this.courseHeading = smoothHeading(
            this.courseHeading,
            movementHeading,
          ...
          this.lastConfirmedMoveAt !== null &&
          now - this.lastConfirmedMoveAt >= MOTION_STOP_TIMEOUT_MS
        ) {
          this.motionConfirmed = false;
          this.motionSpeed = 0;
          this.courseAnchor = fix;
    ...
    const nativeHeading = Number.isFinite(heading)
    ...
    if (this.motionConfirmed) {
      this.motionSpeed =
        this.motionSpeed > 0
          ? this.motionSpeed * 0.65 + providerSpeed * 0.35
          : providerSpeed;
    } else {
      this.motionSpeed = 0;
    }

    const resolvedHeading = Number.isFinite(this.courseHeading)
      ? this.courseHeading
      : nativeHeading;
    ...
    this.onUpdate({
      ...
      speed: this.motionConfirmed ? this.motionSpeed : 0
    });
  }
